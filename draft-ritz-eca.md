...
-   Creates a signed EAT containing identity claims, the Verifier's nonce, and a final `binding_proof`. The `binding_proof` proves knowledge of the secret VF without revealing it while binding it to the procedure context.
...
- Wrong `binding_proof` (fails Gate 9)  
...
| **Key Binding** | `event(VerifierValidatesBinding(bf_2,vf_4,ihb_2,pk,n)) ==> event(AttesterPresentsKey(pk))` | **True** | The final identity key that the Verifier checks is unambiguously bound to the Attester that participated in the attestation procedure, validating **Gate 9 (binding_proof)**. |
...
| `POP_INVALID` | `POP_INVALID` | 9 | The binding_proof (PoP) was invalid. |
